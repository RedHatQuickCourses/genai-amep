...
====
If you already have access to an S3 bucket, an Image Registry (Quay), and the OpenShift AI Model Registry, you can **skip this page** and proceed to xref:modelcar-pipeline.adoc:index.adoc[ModelCar pipeline deployment].
====
...
[source,bash]
----
# 1. Create Namespace (OpenShift Project)
...
.Security Notice
====
The default `mysql-secret.yaml` uses generic passwords (`mysql-admin`). For a production deployment, please rotate these credentials immediately.
====
...
.Security Notice
====
The default `minio-backend.yaml` uses generic passwords (`minio123`). For a production deployment, please rotate these credentials immediately. +
+
The default persistent volume claim for minio is 100Gi, this may not be enough for larger models.
====
...
We use Quay to store our ModelCar images. If you don't have a corporate Quay instance, we will deploy a lightweight standalone version.
 ...
 . Update the `deploy/quay/quay-config-secret-rados.yaml` hostname with the Minio API route.
...
If all pods are `Running`, you are ready to start **Building Modelcar container images**.

xref:modelcar-pipeline.adoc:index.adoc[👉 Go to Modelcar-pipeline]