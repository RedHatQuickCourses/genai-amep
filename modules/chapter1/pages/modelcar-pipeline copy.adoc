...
The ModelCar pipeline automates the workflow of downloading, optionally compressing, packaging, and registering AI models in the RHOAI Model Registry. It uses Tekton to orchestrate the following tasks:
...
* xref:infrastructure-setup.adoc[Infrastructure Setup] - Quay and Model Registry deployed
...
The pipeline requires ConfigMaps for compression and registration scripts:

[source,bash]
----
# Create compress-script ConfigMap
oc create configmap compress-script \
  --from-file=compress.py=deploy/tasks/compress/compress.py \
  -n modelcar-pipelines

# Create register-script ConfigMap
oc create configmap register-script \
  --from-file=register.py=deploy/tasks/register-with-registry/register.py \
  -n modelcar-pipelines
----

=== Create ServiceAccount

The pipeline needs a ServiceAccount for accessing the workspace and registry:

[source,bash]
----
# Create ServiceAccount
oc create serviceaccount modelcar-pipeline -n modelcar-pipelines
----

[NOTE]
====
The ModelCar pipeline focuses solely on packaging and registration. For model deployment and evaluation, use the xref:benchmark-pipeline.adoc[Benchmark Pipeline].
====

=== Deploy Pipeline Tasks

Deploy the individual Tekton tasks that the pipeline references:

[source,bash]
----
# Deploy compression task (optional, only needed if using COMPRESS_MODEL=true)
...
oc get configmaps -n modelcar-pipelines | grep -E "(compress-script|register-script)"
...
. Navigate to Pipelines → PipelineRuns in the OpenShift Console
